    ...
    public class BotGeneratorPatch
    {
        ...
        public class GenerateBotPatch : AbstractPatch
        {
            ...
            public static bool Prefix(BotGenerator __instance, MongoId sessionId, BotBase bot, BotType botJsonTemplate, BotGenerationDetails botGenerationDetails, ref BotBase __result)
            {
                ...
                var vulcanConfig = modConfig.Module.CoreModule.VulcanMod;
                ...
                try
                {
                    ...
                    if (vulcanConfig.Active && vulcanConfig.Config.BotEdit.AlterBoss.Active)
                    {
                        if (botRoleLowercase == "bossknight")
                        {
                            ...
                            if (GoonsCounter.Chance <= vulcanConfig.Config.BotEdit.AlterBoss.Chance.Goons)
                            {
                        ...
                        if (botRoleLowercase == "bosssanitar")
                        {
                            ...
                            if (SanitarCounter.Chance <= vulcanConfig.Config.BotEdit.AlterBoss.Chance.Sanitar)
                            {
                                //命中概率
                                //这里的100是测试, 实际从配置读取
                        ...
                        if (botRoleLowercase == "bossgluhar")
                        {
                            ...
                            if (GluharCocunter.Chance <= vulcanConfig.Config.BotEdit.AlterBoss.Chance.Gluhar)
                            {
                                //命中概率
                                //这里的100是测试, 实际从配置读取
                        ...
                        if (botRoleLowercase == "bosskolontay")
                        {
                            ...
                            if (KolontayCocunter.Chance <= vulcanConfig.Config.BotEdit.AlterBoss.Chance.Kolontay)
                            {
                                //命中概率
                                //这里的100是测试, 实际从配置读取
        ...
        public class AddDogtagToBotPatch : AbstractPatch
        {
            ...
            public static bool Prefix(BotGenerator __instance, BotBase bot)
            {
                ...
                {
                    Id = new MongoId(),
                    Template = VulcanUtil.ConvertHashID("Scav狗牌")
                };
                try
                {
                    ...
                    var dogtag = VulcanUtil.ConvertHashID("Scav狗牌");
                    if (bot.Info.Side != "Savage")
                    {
                    ...
                    else
                    {
                        if (bossList.Contains(botRoleLower))
                        {
                            ...
                            dogtag = VulcanUtil.ConvertHashID("Boss狗牌");
                    ...
                    if (botRoleLower.Contains("sectant"))
                    {
                        dogtag = VulcanUtil.ConvertHashID("邪教徒狗牌");
                        dogTagItem.Upd.Dogtag.Nickname = botName;
        ...
        public static void AddLootToInventory(BotBaseInventory inventory, MongoId itemid, string slot, BotBase bot, DatabaseService databaseService, ISptLogger<BotGenerator> logger)
        {
            ...
            var containerItem = ItemUtils.GetItem(container.Template, databaseService);
            ...
            {
                var itemTemplate = ItemUtils.GetItem(item.Template, databaseService);
