...
exports.initRITCCore = void 0;
const ConfigTypes_1 = require("C:/snapshot/project/obj/models/enums/ConfigTypes");
const config_json_1 = __importDefault(require("../../config.json"));
//
const initRITCCore = (container) => {
    ...
    const packageFolder = vulcanAPI.loadRecursive(packageFolderPath);
    ...
    vulcanAPI.Access("启动核心系统: 罗德岛驻塔科夫贸易中心");
    ...
    for (let pack in packageFolder) {
        ...
        if (active || active == null) {
            ...
            //vulcanAPI.debug(pack)
            //vulcanAPI.debug(packagePath)
            //vulcanAPI.debug(JSON.stringify(info))
            //vulcanAPI.debug(JSON.stringify(packageFolder[pack]))
            const packageItem = vulcanAPI.loadRecursive(`${packagePath}items/`);
            const packageTrader = vulcanAPI.loadRecursive(`${packagePath}traders/`);
            const packageLocale = vulcanAPI.loadRecursive(`${packagePath}res/locale/`);
            ...
            const iconList = vulcanAPI.getFiles(`${packagePath}res/image/`);
...
exports.initRITCCore = initRITCCore;
//# sourceMappingURL=ritc.js.map