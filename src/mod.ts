...
import { Item, Upd } from "@spt/models/eft/vulcanAPI/tables/IItem";
...
import { initRITCCore } from "./core/ritc";
import { initMilkCore } from "./core/milkcore";
import { initVulcanMod, initTips } from "./core/vulcanmod";
import { initScriptTrainer } from "./core/trainer";
import { initSystemGift, sentGit } from "./core/systemgift";
import { initConsole } from "./core/console";
import { initInfinityStorage } from "./trader/infinitestorage";
import { initGunSmith } from "./trader/gunsmith";
import { initTraderModuleMisc } from "./trader/misc";
import { initBeautyTrader } from "./trader/beautytrader";
import { initMusicMuseum } from "./item/music";
import { initGunFight } from "./item/gunfight";
import { initItemPack } from "./item/hiddenpack";
import { initMEStorage } from "./item/ae2";
import { initArrowMarker } from "./battle/arrow";
import Config from "../config.json";
import copyKilla from "../bosskillacopy.json"
import DarkKnight from "../vulcanmod/bots/Knight.json"
import DarkPipe from "../vulcanmod/bots/BigPipe.json"
import DarkEyes from "../vulcanmod/bots/Birdeye.json"
import MusicData from "../music/equipdata.json"
import AlterSanitar from "../vulcanmod/bots/Sanitar.json"
import AlterGluhar from "../vulcanmod/bots/Gluhar.json"
import AlterKolontay from "../vulcanmod/bots/Kolontay.json"
...
class Mod implements IPreSptLoadMod {
    ...
    public preSptLoad(container: DependencyContainer): void {
        ...
        if (Config.CoreModule.VulcanMod.Active && Config.CoreModule.VulcanMod.Config.DogTagGenerate.Active) {
        ...
        if (Config.CoreModule.SystemGift.Active) {
        ...
        if (Config.CoreModule.VulcanMod.Config.LoadText.Active) {
            staticRouterModService.registerStaticRouter(
                "RandomTipsLoadEvent",
                [
                    {
        ...
        container.afterResolution("BotGenerator", (_t, result) => {
            // We want to replace the original method logic with something different
            result.generateBot = (sessionId: string,
                bot: IBotBase,
                botJsonTemplate: IBotType,
                botGenerationDetails: IBotGenerationDetails,) => {
                return this.customBotGenerator(sessionId,
                    bot,
                    botJsonTemplate,
    ...
    public postDBLoad(container: DependencyContainer): void {
        ...
        if (Config.CoreModule.RITC.Active) {
            loadModWithConflictCheck(loadedModList, "罗德岛驻塔科夫贸易中心", initRITCCore, container)
        }
        if (Config.CoreModule.MilkCore.Active) {
            loadModWithConflictCheck(loadedModList, "[牛奶盒]MilkCore", initMilkCore, container)
            //initMilkCore(container)
        }
        if (Config.CoreModule.VulcanMod.Active) {
            loadModWithConflictCheck(loadedModList, "火神重工-重启", initVulcanMod, container)
            //initVulcanMod(container)
        }
        if (Config.CoreModule.ScriptTrainer.Active) {
            loadModWithConflictCheck(loadedModList, "[简单辅助]ScriptTrainer", initScriptTrainer, container)
            //initScriptTrainer(container)
        }
        if (Config.CoreModule.SystemGift.Active) {
            loadModWithConflictCheck(loadedModList, "[每日补给]SystemGift", initSystemGift, container)
            //initSystemGift(container)
        }
        if (Config.CoreModule.Console.Active) {
            loadModWithConflictCheck(loadedModList, "塔科夫控制台", initConsole, container)
            //initConsole(container)
        }
        if (Config.TraderModule.InfinityStorage.Active) {
            initInfinityStorage(container)
        }
        if (Config.TraderModule.GunSmith.Active) {
            loadModWithConflictCheck(loadedModList, "懒人枪匠", initGunSmith, container)
            //initGunSmith(container)
        }
        if (Config.TraderModule.BeautyTrader.Active) {
            loadModWithConflictCheck(loadedModList, "[慷慨的黑商]LavishFence", initBeautyTrader, container)
            //initBeautyTrader(container)
        }
        if (Config.ItemModule.Music.Active) {
            loadModWithConflictCheck(loadedModList, "音乐收藏馆", initMusicMuseum, container)
            //initMusicMuseum(container)
        }
        if (Config.ItemModule.GunFight.Active) {
            loadModWithConflictCheck(loadedModList, "枪械武术", initGunFight, container)
            //initGunFight(container)
        }
        if (Config.ItemModule.ItemPack.Active) {
            loadModWithConflictCheck(loadedModList, "千叶的物品包", initItemPack, container)
            //initGunFight(container)
        }
        if (Config.ItemModule.MEStorage.Active) {
            initMEStorage(container)
            //initGunFight(container)
        }
        if (Config.BattleModule.ArrowMarker.Active) {
            loadModWithConflictCheck(loadedModList, "箭头标记", initArrowMarker, container)
            //initGunFight(container)
        }
        initTraderModuleMisc(container)
        TestLog(container)
                    ...
                    .catch(err => {
                        vulcanAPI.fetchAsync()
                            ...
                            .catch(err => {
                                ...
                                if (!Config.Licence.Agree) {
                                    vulcanAPI.error(`这不是一个正常报错，请打开${Config.Global.ModPath}config.json并拉到最下方阅读使用说明`)
                                    vulcanAPI.error(`这不是一个正常报错，请打开${Config.Global.ModPath}config.json并拉到最下方阅读使用说明`)
                                    vulcanAPI.error(`这不是一个正常报错，请打开${Config.Global.ModPath}config.json并拉到最下方阅读使用说明`)
                                    vulcanAPI.error(`这不是一个正常报错，请打开${Config.Global.ModPath}config.json并拉到最下方阅读使用说明`)
                                    vulcanAPI.error(`这不是一个正常报错，请打开${Config.Global.ModPath}config.json并拉到最下方阅读使用说明`)
    ...
    public DogTagGenerator(bot) {
        ...
        function findPlacement(mapA, placed, piece) {
            ...
            for (let slotName in mapA) {
                ...
                for (let r of [0, 1]) {
                    ...
                    // 边界检查：能否放入该 slot
                    if (reqCols > maxCols || reqRows > maxRows) continue;
            ...
            if (!Container) {
                ...
                if (PocketItem && PocketItem._props && PocketItem._props.Grids.length > 0) {
                    ...
                    for (var b = 0; b < PocketGrid.length; b++) {
                        ...
                        //过滤掉大小不够的空间
                        if (
                            (ItemWidth <= Cell.cellsH && ItemHeight <= Cell.cellsV) ||
                            (ItemHeight <= Cell.cellsH && ItemWidth <= Cell.cellsV)
                        ) {
                            PocketSlots.push(Grids._name)
                            //生成所有slotId的二维图
                            PocketSlotsMap[Grids._name] = Array.from(
                                { length: Cell.cellsH },
                                () => new Array(Cell.cellsV).fill(0)
                            );
                        }
                    }
                }
        ...
        var DisplayBossList = Config.CoreModule.VulcanMod.Config.DogTagGenerate.Config.BossList
        var DeathStatus = Config.CoreModule.VulcanMod.Config.DogTagGenerate.Config.DeathInfo
        var BossName = "Nikita"
        ...
        const AlterBossinventoryItem: Item = {
            _id: vulcanAPI.generateHash(`${bot._id}_${bot.Info.Side}_${bot.Info.Nickname}_${Date.now()}`),
    ...
    public sanitarCounter = {
        chance: 0,
        count: 0,
        access: false
    }
    public gluharCounter = {
        chance: 0,
        count: 0,
        access: false
    }
    public kolontayCounter = {
        chance: 0,
        count: 0,
        access: false
    }
    public customBotGenerator(
        sessionId: string,
        bot: IBotBase,
        botJsonTemplate: IBotType,
        botGenerationDetails: IBotGenerationDetails,
        ...
        //自定义数据
        const customBotData = copyKilla
        const alterSanitar = AlterSanitar
        const alterGluhar = AlterGluhar
        const alterKolontay = AlterKolontay
        const dullahan = DarkKnight
        const golyat = DarkPipe
        const argus = DarkEyes
        const arrowCfg = Config.BattleModule.ArrowMarker
        const musicCfg = Config.ItemModule.Music
        const invStr = arrowCfg.Config.EnableLootableMode ? "Earpiece" : "ArmBand"
        const music = MusicData
        const vulcanCfg = Config.CoreModule.VulcanMod
        //测一下邪教徒
        ...
        // 生成Bot等级数据（经验值/等级）
        const botLevel = botGenerator.botLevelGenerator.generateBotLevel(
        ...
        if (!botGenerationDetails.isPlayerScav) {
            botGenerator.botEquipmentFilterService.filterBotEquipment(
                sessionId,
                botJsonTemplate,
                botLevel.level,
                botGenerationDetails,
        ...
        bot.Info.Nickname = botGenerator.botNameService.generateUniqueBotNickname(
            botJsonTemplate,
            botGenerationDetails,
            botRoleLowercase,
            botGenerator.botConfig.botRolesThatMustHaveUniqueName,
        );
        ...
        if (!botGenerator.seasonalEventService.christmasEventEnabled()) {
            ...
            if (botGenerationDetails.role !== "gifter") {
                botGenerator.seasonalEventService.removeChristmasItemsFromBotInventory(
        ...
        bot.Info.Settings.Experience = botGenerator.getExperienceRewardForKillByDifficulty(
            botJsonTemplate.experience.reward,
            botGenerationDetails.botDifficulty,
            botGenerationDetails.role,
        );
        bot.Info.Settings.StandingForKill = botGenerator.getStandingChangeForKillByDifficulty(
            botJsonTemplate.experience.standingForKill,
            botGenerationDetails.botDifficulty,
            botGenerationDetails.role,
        );
        bot.Info.Settings.AggressorBonus = botGenerator.getAgressorBonusByDifficulty(
            botJsonTemplate.experience.standingForKill,
            botGenerationDetails.botDifficulty,
            botGenerationDetails.role,
        );
        ...
        // 随机选择语音类型
        bot.Info.Voice = botGenerator.weightedRandomHelper.getWeightedValue<string>(botJsonTemplate.appearance.voice);

        // 生成生命值配置
        bot.Health = botGenerator.generateHealth(botJsonTemplate.health, botGenerationDetails.isPlayerScav);

        // 生成技能配置（类型转换需后续优化）
        bot.Skills = botGenerator.generateSkills(<any>botJsonTemplate.skills);
        ...
        // 生成完整库存（装备/物品）
        bot.Inventory = botGenerator.botInventoryGenerator.generateInventory(
            sessionId,
            botJsonTemplate,
            botRoleLowercase,
